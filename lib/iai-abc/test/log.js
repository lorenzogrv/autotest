...
test('Log prototype', function( t ){
  ...
  t.equal( Log.stdout, process.stdout, 'should default stdout to process' );
  t.equal( Log.stderr, process.stderr, 'should default stderr to process' );
  t.equal( Log.muted, false, 'should default muted to false' );
  t.equal( Log.clean, false, 'should default clean to false' );
...
test('log instances', function ( t ){
  ...
  t.equal( log.filename, __filename, 'should be bound to a filename' );
  t.equal( log.level, Log.WARN, 'default level to WARN' );
  t.equal( log.muted, false, 'should be unmuted by default' );
  t.deepEqual( log.stdout, Log.stdout, 'default stdout to prototype stdout' );
  t.deepEqual( log.stderr, Log.stderr, 'default stderr to prototype stdout' );
...
test('log instances (bifid output)', function( t ){
  function testit( method, yes, no ){
    yes.write = t.pass.bind(t, 'Log.'+method+' writes to the right stream');
    no.write = t.fail.bind(t, 'Log.'+method+' writes to the wrong stream');
    Log[method]('some message');
  }
  var err = new PassThrough(), out = new PassThrough();
  Log.stdout = out; Log.stderr = err;

  // 1 assertion for each log level except "fatal" (3 for that one)
  t.plan( 4 + 3 );

  Log.level = Log.VERB; // set level to verbose to check "verb" method

  // verb and info must output to "stdout", not "stderr"
  testit( 'verb', out, err ); testit( 'info', out, err );
  // warn and error must output to "stderr", not "stdout"
  testit( 'warn', err, out ); testit( 'error', err, out );

  // restore Log output streams before test ends
  Log.stdout = process.stdout; Log.stderr = process.stderr;

  // customize code to check Log.fatal exits with proper code
  var code = 21, lines = [
...
test('log instances (mute behaviour)', function( t ){
  // 1 assertion for each log level except "fatal" (3 for that one)
  t.plan( 4 + 3 );
  Log.muted = false; // ensure it's not muted before
  function testit( stream, method ){
    stream.write = t.pass.bind(t, 'Log.'+method+' writes to stdout');
    Log[method]('some message');
    Log.muted = true;
    stream.write = t.fail.bind(t, 'muted but got output with Log.'+method);
    Log[method]('some message');
    Log.muted = false;
  }
  var err = new PassThrough(), out = new PassThrough();
...
test('log instances (log-level behaviour)', function( t ){
  // fake output streams
  var out = new PassThrough(), err = new PassThrough();
  Log.stdout = out; Log.stderr = err;
  ...
  when( 'VERB' )
    ...
    .stream( err ).should( 'pass', 'warn' ).should( 'pass', 'error' )
  ;
  ...
  when( 'ERROR' )
    .stream( out ).should( 'fail', 'verb' ).should( 'fail', 'info' )
    .stream( err ).should( 'fail', 'warn' ).should( 'pass', 'error' )
  ;
  // FATAL level should block output from ¿error?, warn, info, and verb
  when( 'FATAL' )
    .stream( out ).should( 'fail', 'verb' ).should( 'fail', 'info' )
    .stream( err ).should( 'fail', 'warn' ).should( 'fail', 'error' )
  ;
...
test('log instances (configurable default streams)', function( t ){
  var log = Log.constructor();

  // when Log's default streams are changed...
  var out = Log.stdout = new PassThrough();
  var err = Log.stderr = new PassThrough();
  log = Log.constructor();
  t.deepEqual( log.stdout, out, 'default stdout should be the new default' );
  t.deepEqual( log.stderr, err, 'default stderr should be the new default' );

  Log.stdout = err;
  var log2 = Log.constructor();
  t.deepEqual( log2.stdout, err, 'new instances get the newest defaults' );
  t.deepEqual( log.stdout, out, 'but old instances stay with its state' );
